...
class FullyConnectedNetwork(nn.Module):
    def __init__(self, input_dim, config, output_dim=2):
        ...
        self.features = nn.ModuleDict(layers_dict)
    ...
    def _make_layer(self):
        ...
        for x in self.config[:-1]:
            if x == 'M':
                ...
                #WARNING: be sure to print the names of modules before use them, the naming is pretty messy
            else:
                layers_dict[f"conv{idx}"] = nn.Conv2d(in_channels, x, kernel_size=3, padding=1)
                layers_dict[f"batchnorm{idx}"] = nn.BatchNorm2d(x)
